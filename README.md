...
Self-hosted WordPress update server for Alltime Technologies plugins, served
as a static site via GitHub Pages at
`https://alltimeuk.github.io/wp-updates/`.
...
5. Add an entry to `products.json` at the repo root:
   ```json
   {
     ...
     "homepage": "https://alltimetech.co.uk"
   }
   ```
6. Point the plugin's updater at
   `https://alltimeuk.github.io/wp-updates/<slug>/update.json`.
